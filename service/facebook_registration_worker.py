...
from service.proxy_manager import ProxyConfigManager
from utils.u2_device_manager import U2DeviceManager
from service.proxy_manager import WWProxyHandler
...
class FacebookRegistrationWorker:
    ...
    def get_proxy_config_from_main_config(self) -> Dict[str, Any]:
        """Lấy proxy config từ main config đã được truyền vào"""
        try:
            if hasattr(self, 'main_config') and self.main_config:
                return self.main_config.get('proxy_config', {})
            else:
                return ProxyConfigManager.get_proxy_config_from_gui()
        except Exception as e:
    ...
    def process_device(self, device_id: str, device_index: int):
        try:
            ...
            if proxy_type and proxy_type in proxy_mapping:
                ...
                self.status_manager.update_device_status(device_index, {"stt": stt_display, "trang_thai": "Đang tiến hành setup Proxy", "ten_may": device_id, "ket_qua": "", "ho": "", "ten": "", "mat_khau": "", "email_sdt": "", "uid": "", "cookie": "", "token": "", "proxy": ""}, self.table_manager)
                ...
                self.status_manager.update_device_status(device_index, {"stt": stt_display, "trang_thai": f"Proxy type không xác định: {proxy_type}", "ten_may": device_id, "ket_qua": "Lỗi", "ho": "", "ten": "", "mat_khau": "", "email_sdt": "", "uid": "", "cookie": "", "token": "", "proxy": ""}, self.table_manager)
            ...
            for fb_attempt in range(3):
                ...
                if d.xpath('//*[@content-desc="Meta logo" or @content-desc="Logo Meta"]').exists:
                    ...
                    time.sleep(1)
                    d.xpath('//*[@text="Create new account"] | //*[@text="Tạo tài khoản mới"] | //*[@text="Bắt đầu"]').click()
            ...
            # Xử Lý Proxy
            # Mapping proxy_type với handler và function
            proxy_mapping = {"khong_dung_proxy": {"handler": "NoProxyHandler", "function": ""}, "host_port_user_pass": {"handler": "HostPortUserPassHandler", "function": "setup_proxy"}, "host_port": {"handler": "HostPortNenDungHandler", "function": "setup_proxy"}, "vn2ray": {"handler": "Vn2rayHandler", "function": "nhapip"}, "ww_proxy": {"handler": "WWProxyHandler", "function": "change_ip"}, "proxy_no1": {"handler": "ProxyNo1Handler", "function": "setup_proxy"}, "super_proxy": {"handler": "SuperProxyHandler", "function": "setup_proxy"}, "windscribe_vpn": {"handler": "WindscribeVPNHandler", "function": "setup_proxy"}}
